...
\begin{frame}{4.3 Lagrangian and first-order conditions}
  ...
  $\mu$ is the \textbf{Lagrange multiplier}.
\end{frame}
...
\begin{frame}{4.3 Sensitivity without the Lagrangian}
  ...
  Substitute the FOC:
  \[
    df=\nabla f\cdot dx=\mu\,\nabla h\cdot dx=\mu\,dc.
  \]
  
  Hence $\displaystyle\frac{dV}{dc}=\mu$: the multiplier is the sensitivity of $f$ to relaxing the constraint.
\end{frame}

\begin{frame}{4.3 Connect to the Lagrangian}
  \small
  Same problem: $\max_x f(x)$ s.t.\ $h(x)=c$, with $\mathcal{L}(x,\mu;c)=f(x)+\mu\bigl(c-h(x)\bigr)$.
  
  Let $V(c)=f(x^*(c))$. Binding constraint $\Rightarrow$ $V(c)=\mathcal{L}(x^*(c),\mu^*(c);c)$.
  
  Chain rule: FOCs kill $\nabla_x\mathcal{L}\cdot dx^*/dc$ and $(\partial\mathcal{L}/\partial\mu)\,d\mu^*/dc$, so
  \[
    \frac{dV}{dc}=\frac{\partial\mathcal{L}}{\partial c}.
  \]
  
  And $\partial\mathcal{L}/\partial c=\mu^*$ by inspection of $\mathcal{L}$.
  
  \textbf{Summary.} $\displaystyle\frac{dV}{dc}=\frac{\partial\mathcal{L}}{\partial c}=\mu^*$.
\end{frame}
...
\QFrame{4.3 Exercise: Cobb--Douglas with budget}
  ...
  From the FOC, $\mu = \frac{\partial u/\partial x_1}{p_1}$: $\mu$ is the utility gain from spending one more dollar on $x_1$
  (same for $x_2$ at the optimum).
\end{frame}

\QFrame{4.3 Exercise: Interpreting the multiplier}
  \small
  \textbf{Exercise.} In the Cobb--Douglas solution, compute $\mu^*$ and interpret it.
  
  \textbf{Solution.} From FOC and $p_1x_1=\alpha w$: $\mu=u/w$. Plug in demands:
  \[
    \mu^*=\frac{\alpha^\alpha(1-\alpha)^{1-\alpha}}{p_1^\alpha p_2^{1-\alpha}}.
  \]
  
  In general we only know $\mu^*=dV/dw$ (shadow price). Here something stronger holds: $\mu^*=u^*/w=V/w$.
  
  That is CD-specific: MUI equals \emph{average} utility per dollar, not just the marginal.
  
  Since the explicit $\mu^*$ does not depend on $w$, integrate $dV/dw=\mu^*$ with $V(0)=0$ to get $V(w)=\mu^* w$, which is consistent with $\mu^*=V/w$.
\end{frame}
...
\THFrame{4.3 Exercise: Cost minimization}
  ...
  Same logic: $\mu = dC^*/d\bar{y}$, the marginal cost of relaxing the output constraint.
\end{frame}
...
\QFrame{4.4 Exercise: Roy's identity}
  ...
  Ratio: $\frac{-\lambda^* x_i^*}{\lambda^*}=-x_i^*$.

  \textbf{Intuition.} The channel $p_i\to x^*\to V$ is zero to first order at an optimum (FOC / envelope).
  What remains is direct: a higher $p_i$ costs about $x_i^*$ more dollars, each worth $\lambda^*=\partial V/\partial w$ utils,
  so $\partial V/\partial p_i=-\lambda^* x_i^*$. Divide by $\partial V/\partial w$ to recover $-x_i^*$.
\end{frame}
