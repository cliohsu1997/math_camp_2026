...
\THFrame{4.2 Exercise: Testing quasiconcavity}
  ...
  \textbf{Strategy.} Use Definition B: show upper contour sets $S_t = \{(x,y) \in \mathbb{R}^2_{++} : xy \geq t\}$ are convex for every $t$.
  
  \textbf{Solution.}
  
  \begin{itemize}
    \item If $t \leq 0$: every point in $\mathbb{R}^2_{++}$ has $xy > 0 \geq t$, so $S_t = \mathbb{R}^2_{++}$, which is convex
    
    \item If $t > 0$: $xy \geq t$ is equivalent to $\ln x + \ln y \geq \ln t$
    
    \item Let $g(x,y) = \ln x + \ln y$. Then $S_t = \{(x,y) : g(x,y) \geq \ln t\}$ is a \textbf{superlevel set} of $g$
    
    \item $g$ is strictly concave on $\mathbb{R}^2_{++}$: $H_g = \mathrm{diag}(-1/x^2,\,-1/y^2)$ is negative definite
    
    \item Superlevel sets of a concave function are convex, so each $S_t$ is convex
  \end{itemize}
  
  Therefore $f(x,y)=xy$ is quasiconcave (in fact \textbf{log-concave}, since $\ln f = g$ is concave and $f>0$).
\end{frame}
