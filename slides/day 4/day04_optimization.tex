...
\begin{frame}{4.5 Sign convention summary}
  ...
  \textbf{Memory rule.} Prefer writing constraints as $g\geq 0$ when maximizing:
  max (``$+$'' in $\mathcal{L}$) and $g\geq 0$ (``positive'' slack) $\Rightarrow$ $\lambda\geq 0$.
  Two positives give a positive multiplier.
  
  Equivalently: if you keep $g\leq 0$ under max, then $\lambda\leq 0$ (same inequality direction).
\end{frame}
...
\QFrame{4.5 Exercise: Budget with nonnegativity}
  ...
  \begin{itemize}
    ...
    \item Lagrangian: $\mathcal{L} = u + \lambda_1(x_1 + x_2 - 1) + \lambda_2(-x_1) + \lambda_3(-x_2)$
    
    \item All $\lambda_j \leq 0$ (max, $g_j\leq 0$, $+$ convention)
    
    \item Stationarity: $\frac{\partial u}{\partial x_1} + \lambda_1 - \lambda_2 = 0$, $\frac{\partial u}{\partial x_2} + \lambda_1 - \lambda_3 = 0$
    
    \item CS: $\lambda_1(x_1 + x_2 - 1) = 0$, $\lambda_2(-x_1) = 0$, $\lambda_3(-x_2) = 0$
  \end{itemize}
\end{frame}
...
\QFrame{4.5 Exercise: Corner solution analysis}
  ...
  \begin{itemize}
    \item At $(0, 1)$: $g_1$ binds ($\lambda_1 \leq 0$), $g_2$ binds ($\lambda_2 \leq 0$), $g_3$ slack ($\lambda_3 = 0$)
    ...
    \item But then $\lambda_2 = 1 > 0$, violating dual feasibility $\lambda_2 \leq 0$
  \end{itemize}
...
\begin{frame}{4.6 The key difference: two-sided vs. one-sided}
  ...
  \begin{itemize}
    \item Inequality multipliers need sign restrictions (with $\mathcal{L}=f+\sum\lambda_j g_j$: $\lambda\leq 0$ for max, $g\leq 0$)
    
    \item Equality multipliers are unrestricted ($\lambda \in \mathbb{R}$)
  \end{itemize}
\end{frame}
...
\begin{frame}{4.6 Formal explanation}
  ...
  \begin{itemize}
    ...
    \item No sign restriction needed on $\lambda$
  \end{itemize}
  ...
  \begin{itemize}
    ...
    \item Need $\lambda \leq 0$ so that $\nabla f=-\lambda\nabla g$ lies in the outward-normal cone $K$
  \end{itemize}
...
\begin{frame}{4.6 Example: Comparing equality vs. inequality}
  ...
  \begin{itemize}
    ...
    \item Optimum at $(0.5, 0.5)$ with $\lambda = -0.5$ if $\mathcal{L}=xy+\lambda(x+y-1)$ (sign unrestricted)
  \end{itemize}
  ...
  \begin{itemize}
    ...
    \item Same optimum, now $\lambda \leq 0$ (max, $g\leq 0$, $+$ convention)
    
    \item Geometric: $\nabla f$ must lie in the cone of active $\nabla g_j$
  \end{itemize}
\end{frame}
...
\begin{frame}{4.6 The combined Lagrangian}
  ...
  \[
    \mathcal{L} = f + \underbrace{\sum_k \lambda_k h_k}_{\text{equality}} + \underbrace{\sum_j \lambda_j g_j}_{\text{inequality}}.
  \]
  ...
  \begin{center}
    \begin{tabular}{@{}ll@{}}
      ...
      \midrule
      Equality $\lambda_k$ & \textbf{unrestricted} (explained in this section) \\
      Inequality $\lambda_j$ & $\lambda_j \leq 0$ if $g_j \leq 0$; $\lambda_j \geq 0$ if $g_j \geq 0$ \\
      \bottomrule
    \end{tabular}
  \end{center}
  
  \textbf{Short rule.} For max: write $g\geq 0$ $\Rightarrow$ $\lambda\geq 0$ (two positives).
  If instead $g\leq 0$, then $\lambda\leq 0$.
\end{frame}
...
\QFrame{4.6 Exercise: Comparing equality vs. inequality}
  ...
  \begin{itemize}
    \item Lagrangian: $\mathcal{L} = x^2 - y + \lambda(1 - x^2 - y^2)$
    
    \item FOCs: $2x - 2\lambda x = 0$, $-1 - 2\lambda y = 0$
    
    \item From first: $x = 0$ or $\lambda = 1$
    
    \item If $\lambda = 1$: $y = -0.5$, $x = \pm \sqrt{0.75}$
  \end{itemize}
  
  Here $\lambda = 1 > 0$, but could be negative for different objectives.
  
  The unrestricted $\lambda$ allows both increasing and decreasing the objective by moving along the circle.
\end{frame}
...
\THFrame{4.6 Exercise: Unrestricted in action}
  ...
  \begin{itemize}
    ...
    \item FOC: $2x - \lambda = 0$, so $\lambda = 2$ at $x = 1$
  \end{itemize}
  
  \textbf{Inequality $x \geq 1$ (write $g=1-x\leq 0$):}
  
  \begin{itemize}
    \item For min with $g\leq 0$: $\lambda \geq 0$
    
    \item $\mathcal{L} = x^2 + \lambda(1 - x)$
    
    \item FOC: $2x - \lambda = 0$, so $\lambda = 2 \geq 0$ at $x=1$ --- dual feasibility holds
  \end{itemize}
  
  Equality: $\lambda$ free. Inequality: $\lambda$ must obey the sign rule for min.
\end{frame}
