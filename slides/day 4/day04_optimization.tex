...
\begin{frame}{4.2 Multivariate concavity: Three definitions}
  ...
  \textbf{(3) Hessian ($C^2$).} $H_f(\mathbf{x})$ is \textbf{negative semidefinite} for all $\mathbf{x}$ (defined on next slide).
  ...
  \hyperlink{appendix:multivariate_concave}{\scriptsize $(3)\Rightarrow(2)$, $(1)\Rightarrow(3)$: A.4 \InterestOnly}
\end{frame}
...
\begin{frame}{4.2 Concavity and the Hessian}
  \small
  \textbf{Theorem.} For $f \in C^2$: tangent characterization \textbf{(2)} $\Leftrightarrow$ Hessian NSD \textbf{(3)} everywhere.
  
  Equivalently: $f$ is concave iff $H_f(\mathbf{x})$ is NSD at every $\mathbf{x}$.
  
  \textbf{Proof directions.}
  \begin{itemize}
    \item \textbf{$(3) \Rightarrow (2)$:} Taylor + NSD $\Rightarrow$ tangent inequality
    
    \item \textbf{$(1) \Rightarrow (3)$:} symmetric chord concavity + Taylor $\Rightarrow$ NSD
    
    \item Since $(1)\Leftrightarrow(2)$ (A.2), this gives $(2)\Leftrightarrow(3)$
  \end{itemize}
  
  \hfill\hyperlink{appendix:multivariate_concave}{\scriptsize Proofs: Appendix A.4 \InterestOnly}
\end{frame}
