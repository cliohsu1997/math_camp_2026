...
\begin{frame}{4.6 Key intuition: one-sided vs.\ two-sided wall}
  ...
  For a max, binding is two steps:

  \begin{enumerate}
    \item \textbf{Binding} $\Rightarrow$ moving with $\nabla g$ improves $f$:
      $\nabla f\cdot\nabla g>0$ (same open half-space).
    \item \textbf{Binding + stationarity} (one active $g$)
      $\Rightarrow$ same ray
      $\Rightarrow$ $\nabla f=-\lambda\nabla g$ with $-\lambda>0$
      (equivalently $\lambda<0$).
  \end{enumerate}
  Step 1 is geometry; step 2 is the FOC (the wall holds $f$ back).
