...
\title{Math Bootcamp}
\subtitle{Day 1 --- Foundations (Sections 1.1--1.5)}
...
\begin{frame}{1.5 Supremum and infimum --- names and definitions}
  ...
  \medskip
  \textbf{In our example:}\quad
  $\sup P=179.9$, but $\sup P\notin P$ --- so no $\max P$.

  \medskip
  {\scriptsize Use $\sup$ when a bound exists but $\max$ may not ---
  reservation prices, demand choke prices, limits in equilibrium.}
\end{frame}
