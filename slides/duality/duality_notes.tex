...
Before computing an optimum, it is useful to ask a simpler question: how low can cost \emph{possibly} be,
given only the constraints and nonnegativity of $\mathbf{x}$?
...
The next section makes this precise for linear programs: we derive one primal--dual pair, prove that dual
values are always valid lower bounds, and then show that the best dual value equals the primal optimum.
...
The \textbf{primal} in resource-requirement form is
...
A small illustration fixes the notation.
Consider $\min\ 3x_1+5x_2$ s.t.\ $x_1+2x_2\ge 4$, $x_1,x_2\ge 0$
