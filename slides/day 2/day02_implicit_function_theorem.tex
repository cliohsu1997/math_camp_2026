...
\title{Math Bootcamp}
\subtitle{Day 2 --- More calculus and a bit of linear algebra}
...
\begin{frame}{2.2 Example --- labor and output (economics)}
  \footnotesize
  \textbf{Setup.}\ A firm produces $f(L)$ units of output using labor $L$.
  Suppose $f$ is continuous on $[L_1,L_2]$ and differentiable on $(L_1,L_2)$.

  \medskip
  \textbf{MVT.}\ $\exists\,L^*\in(L_1,L_2)$ such that
  \[
    f'(L^*)=\frac{f(L_2)-f(L_1)}{L_2-L_1}.
  \]

  \medskip
  \textbf{Interpretation.}\ At some interior input level, the \textbf{marginal product of labor}
  equals the \textbf{average product} over the whole change from $L_1$ to $L_2$.

  \medskip
  {\scriptsize Same idea for cost: on $[0,q]$, some $q^*$ has $C'(q^*)=C(q)/q$
...
\begin{frame}{A.2 Expectation from the CDF}
...
\begin{frame}{A.3 Leibniz rule --- failure example}
...
\begin{frame}[plain]{A.3 Leibniz failure --- moving step (graph)}
  \centering
  \vfill
...
\begin{frame}{A.4 Taylor expansion (one variable)}
...
\begin{frame}{A.5 What is Brownian motion?}
  \footnotesize
...
\begin{frame}{A.5 Taylor and It\^o's lemma}
...
\begin{frame}{A.5 It\^o multiplication rules}
  \footnotesize
