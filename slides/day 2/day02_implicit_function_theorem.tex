...
\begin{frame}{2.1 Egg market --- a feed-cost shock}
  ...
  \end{center}
  {\footnotesize This figure: \textbf{positive} cost shock $=$
  \textbf{negative} supply shock ($S$ left; $p^*$ up, $q^*$ down).
  Converse: \textbf{negative} cost shock $=$ \textbf{positive} supply shock
  ($S$ right).}
\end{frame}
...
\begin{frame}{2.1 No closed form --- nonlinear market}
  ...
  \[
    ...
    \alpha>0.
  \]
  Clearing: $\alpha e^{-p}=p$, or $E(p,\alpha)=\alpha e^{-p}-p=0$.

  \medskip
  This equation \textbf{defines} $p^*(\alpha)$, but it has
  \textbf{no elementary closed form}.
  We cannot write $p^*=g(\alpha)$ and differentiate.

  \medskip
  We still want $\dfrac{dp^*}{d\alpha}$.
  That is the problem the Implicit Function Theorem solves.
\end{frame}
...
\QFrame{2.1 --- Apply IFT (no closed form)}
  \scriptsize
  \textbf{Nonlinear demand:}\ $D(p,\alpha)=\alpha e^{-p}$,\ $S(p)=p$,\ $\alpha>0$.

  \textbf{No elementary closed form for $p^*(\alpha)$}. Use IFT.
...
\begin{frame}{2.1 Example --- tax incidence}
  ...
  \medskip
  \textbf{Incidence (per \$1 of tax).}\
  Buyer price rises by $\dfrac{d}{b+d}$; seller net price falls by $\dfrac{b}{b+d}$.
  The tax is borne \textbf{more heavily by the less elastic side}
  (small $b$: inelastic demand $\Rightarrow$ buyers pay most;
  small $d$: inelastic supply $\Rightarrow$ sellers pay most).
\end{frame}
