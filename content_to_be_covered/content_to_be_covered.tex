...
\begin{itemize}[leftmargin=*]
    ...
    \item Level curves / contour map: $\mathcal{L}_c=\{(x,y):f(x,y)=c\}$;
      3D picture as a horizontal slice of $z=f(x,y)$.
...
\begin{itemize}[leftmargin=*]
    \item Exact change: $\Delta z=f(x_0+\Delta x,y_0+\Delta y)-f(x_0,y_0)$.
    \item Add/subtract trick at $(x_0+\Delta x,y_0)$; change one variable at a time.
    \item MVT on each bracket $\Rightarrow f_x\,\Delta x+f_y\,\Delta y$; limit
      $\Delta x,\Delta y\to 0$ gives $dz=f_x\,dx+f_y\,dy$.
    \item 3D picture: L-shaped path on the surface ($\Delta x$ with $y$ fixed, then
      $\Delta y$ with $x$ fixed).
    \item Application: $dY=MP_K\,dK+MP_L\,dL$; later $dU$ and budget differentials.
\end{itemize}

\subsection{Gradient Geometry}
\begin{itemize}[leftmargin=*]
    \item Contour-map picture: $\nabla f$ at $P$ on $\mathcal{L}_c$, tangent to level curve.
    \item Derivation ($\perp$): if $\mathbf{r}(t)\in\mathcal{L}_c$, then
      $0=\frac{d}{dt}f(\mathbf{r}(t))=\nabla f\cdot\mathbf{r}'(t)$.
    \item Derivation (uphill): $D_{\mathbf{u}}f=\nabla f\cdot\mathbf{u}=\|\nabla f\|\cos\theta$;
      maximum when $\mathbf{u}\parallel\nabla f$.
    \item Along a level curve, $D_{\mathbf{u}}f=0$; MRS as slope of indifference curve.
\end{itemize}

\subsection{Directional Derivatives}
\begin{itemize}[leftmargin=*]
    \item Rate of change of $f$ along a chosen unit direction $\mathbf{u}$
      (rate version of $df$ with $(dx,dy)=t\mathbf{u}$).
    \item Definition as a limit along the line $(a,b)+t\mathbf{u}$.
    \item Derivation: same chain rule as $df$; $\varphi(t)=f(a+tu_1,b+tu_2)$
      $\Rightarrow D_{\mathbf{u}}f=\nabla f\cdot\mathbf{u}$.
\end{itemize}
...
\midrule
1 & Notation; logic; sets; functions & Sup/inf; homogeneous functions; limits & Derivatives; partials; total differential; directional \\
