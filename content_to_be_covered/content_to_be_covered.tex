...
\begin{itemize}[leftmargin=*]
    ...
    \item \textbf{Matrices:} dimensions $m\times n$, entries, matrix--vector product $A\mathbf{x}$;
      addition, scalar multiplication, matrix multiplication (order matters).
    \item \textbf{Transpose} $A^{\mathsf T}$; identity matrix $I$.
    ...
    \item \textbf{Determinant} (must know $2\times 2$ and $3\times 3$ formulas):
    ...
    \item \textbf{Jacobian matrix:} $J_f(\mathbf{x})=\bigl(\partial f_i/\partial x_j\bigr)$;
      local linear map of a multivariable function; link to Day~2 IFT appendix.
    \item \textbf{Quadratic forms} $\mathbf{x}^{\mathsf T}A\mathbf{x}$ (stated only):
      positive/negative definiteness and curvature of $f$ near a critical point (preview Day~4 SOC).
    \item \textbf{IFT in matrix form:} if $\mathbf{F}(\mathbf{y},\boldsymbol{\theta})=\mathbf{0}$
      and $\partial\mathbf{F}/\partial\mathbf{y}$ is invertible, then
      $\partial\mathbf{y}^*/\partial\boldsymbol{\theta}=
      -\bigl(\partial\mathbf{F}/\partial\mathbf{y}\bigr)^{-1}\bigl(\partial\mathbf{F}/\partial\boldsymbol{\theta}\bigr)$.
\end{itemize}
